<template>
  <div class="withdraw-deposit sub-page">
        ...
        <div class="pay-item" v-for="item in cateList" :key="item.id">
          ...
          <div class="right-box">
            ...
            <div class="right-wrap">
              <van-button @click="handlePay(item)" :disabled="item.r_status !== 1">
                <span v-if="item.r_status === 0">{{ $t('未缴纳') }}</span>
                <span v-else-if="item.r_status === 1">{{ $t('提取') }}</span>
                <span v-else><van-count-down :time="(item.handle_time - item.now_time) * 1000" @finish="handleFinish"/></span>
              </van-button>
            </div>
          </div>
        </div>
    ...
    <result-dialog :visible.sync="showDepositResult" :result="$t('提取押金成功')" :btn="true"></result-dialog>
  </div>
</template>
