<template>
  <div class="deposit-confirm sub-page">
    <nav-bar :title="$t('充值确认')" left-arrow></nav-bar>
    <div class="container">
      ...
      <div class="address-box">
        ...
        <van-field v-model="address" type="text" readonly>
          <template #button>
            <van-button class="copy" @click="handleCopyLink">{{ $t('复制') }}</van-button>
          </template>
        </van-field>
      </div>
  ...
  methods: {
    ...
    async handleUpload(file) {
      ...
      try {
        ...
        const res = await this.$api.home.upload(form)
        if (res.code === 1) {
