<template>
  <div class="withdraw sub-page">
    ...
    <van-dialog v-model="visible" :show-confirm-button="false" width="295px" close-on-click-overlay>
      <div class="dialog-container">
        <div class="title">{{ $t('取款密码') }}</div>
        <div class="field-box">
          <van-field v-model="pwd" type="password" :placeholder="$t('请输入取款密码')"></van-field>
        </div>
        <div class="btn-box">
          <van-button block @click="handleWithdraw" :loading="buttonLoading"
                      loading-type="spinner" :loading-text="$t('正在提现...')">{{ $t('确定') }}</van-button>
        </div>
      </div>
    </van-dialog>
    <result-dialog :visible.sync="showResult" :result="$t('提现成功')"></result-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    async withDraw () {
      try {
        this.loading = true
        const resp = await this.$api.home.withDraw();
        if (resp.code === 1) {
    ...
    async handleWithdraw () {
      ...
      try {
        ...
        const res = await this.$api.home.withdrawal(params)
        if (res.code === 1) {
