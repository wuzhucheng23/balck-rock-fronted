...
const _tail = Symbol('linkListTail');
...
export default class DoublyLinkList {
    constructor() {
        ...
        this[_tail] = null;   // 表尾元素（其他元素都是通过此元素来查找）
    }
    ...
    insert(element, index) {
        if (index >= 0 && index <= this[_count]) {    // 若索引有效
            ...
            if (index === 0) {  // 若插入的是表头元素
                if (this.head === null) {   // 若链表为空
                    this[_head] = node;
                    this[_tail] = node;
                } else {
                    node.next = current;
            ...
            } else if (index === this[_count]) {    // 若插入是表尾元素
                current = this[_tail];
                current.next = node;
                node.prev = current;
                this[_tail] = node;
    ...
    removeAt(index) {
        if (index >= 0 && index < this[_count]) {     // 若索引有效
            let current = this[_head];
            if (index === 0) {  // 若删除的是表头元素
                this[_head] = current.next;
                if (this[_count] === 1) {   // 若链表只有一个元素
                    this[_tail] = null;
                } else {
